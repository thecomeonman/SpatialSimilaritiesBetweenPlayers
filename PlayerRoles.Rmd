...
I further broke down each cluster group into smaller clusters, such that each cluster group, except for the GK one, had at least two clusters in it. Two because that might at least pick out players in roles which more attacking or more defensive although there is no guarantee that that's how the two clusters would split the cluster group they belong to. Given that the clustering is hierarchical, we can continue to break the clusters down even more and get more and more granular definitions.

## Cluster Group Differences {.tabset .tabset-fade .tabset-pills}
...
This cluster group is right wingers, divided into two clusters, cluster 2 is a more attacking cluster and cluster 21 is a more defensive cluster.
...
* Cluster 3 might be the central defender of the 3 at the back, or the more forward foraying centre back. The algorithm thinks this player is more similar to midfielders than other centre backs interestingly.
...
* Cluster 4 is more involved in the centre of the pitch that cluster 6 which tends to stick to the wing
...
* Cluster 5 has a bias towards deeper in the pitch. There is a higher occurrence of receiving passes from the deep so I'd speculate that these are forwards trying to win long balls.
...
This cluster group has mostly right sided midfielders. Cluster 9 is slightly more central and defensive, and probably the RCM of teams playing with two central midfielders. cluster 13 is wider and more attacking, probably the RCM of teams playing with three midfielders.

```{r child = '/tmp/ClusterGroup8.Rmd'}
```

This cluster group has mostly RBs. Clusters 10 and 25 probably correspond to more advanced RBs and more defensive RBs respectively.
...
This cluster group has mostly left sided midfielders. cluster 12 is wider and more attacking, probably the LCM of teams playing with three midfielders. Cluster 22 is slightly more central and defensive, and probably the LCM of teams playing with two central midfielders.
...
* Cluster 2 is a more free roaming sort of role with plenty of activity towards the left of the pitch as well compared to the cluster group.

* Cluster 23 is a deeper, more central sort of role with a higher proportion of touches behind the centre line compared to the cluster group.

* Cluster 28 is a role of a player pushed higher up and towards the corner.
...
* Cluster 56 is similar to cluster 23, with a lot of activity earlier in the pitch, but players in this role tend to stay closer to the touchline compared to what we saw with cluster 23.

* Cluster 61 is similar to but even more defensive than cluster 23.

* Cluster 83 is not deployed as far wide as the other clusters. The bump in passes received from various spots deeper in the pitch is interesting.

* Cluster 86 is probably the stereotypical right winger, with lots of activity on the touchline and in the attacking half.

* Cluster 92 looks like wingbacks, given how their activity is well spread across the wing and not as concentrated in certain areas as what the other clusters showed.
